...
app.use(
  session({
    ...
    secret: process.env.SESSION_SECRET || 'card-monitor-dev-secret-change-me',
    resave: false,
...
app.post('/api/auth/logout', (req, res) => {
  req.session.destroy((err) => {
    ...
    res.clearCookie('cardMonitor.sid');
    res.json({ ok: true });
  });
});
...
app.post('/api/import/upload', requireAuth, upload.single('file'), (req, res) => {
  try {
    ...
    console.error(e);
    res.status(500).json({ error: e.message || '업로드 처리 실패' });
  }
});
...
app.post('/api/import/sample', requireAuth, (_req, res) => {
  try {
    ...
    console.error(e);
    res.status(500).json({ error: e.message || '샘플 생성 실패' });
  }
});
...
app.get('/api/import/batches/:id', requireAuth, (req, res) => {
  try {
    const data = getBatchWithRows(req.params.id);
    if (!data) {
